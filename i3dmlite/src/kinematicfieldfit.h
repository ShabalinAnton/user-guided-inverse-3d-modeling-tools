...
// this is the library that backs the stationary sweep fitting module
...
// This class provides the functions to solve for the parameters of a kinematic field (of type specified by LinFieldEval)
    ...
    struct LinParamKinematicFieldFitter {
        ...
        void fitKinematicField(DataIterator first, DataIterator last, std::vector<double> &parametersOut, double w_p = 0)
        {
            clear();
            if (w_p == 0) {
                for (DataIterator i = first; i != last; i++) { addElement(i->w, i->p, i->n); }
            } else {
                for (DataIterator i = first; i != last; i++) { addElement(i->w, i->p, i->n, w_p); }
            }
    ...
    void LinParamKinematicFieldFitter<LinFieldEval>::fitKinematicField(TriangleMesh &mesh, std::vector<double> &parametersOut)
    {
        ...
        fitKinematicField(dunavantIterator(mesh), dunavantIterator::end(mesh), parametersOut);
    }
    ...
    void LinParamKinematicFieldFitter<LinFieldEval>::fitKinematicField_HEIV(TriangleMesh &mesh, std::vector<double> &parametersOut, int iters, double w_p)
    {
        ...
        fitKinematicField_HEIV(dunavantIterator(mesh), dunavantIterator::end(mesh), parametersOut, iters, w_p);
    }
